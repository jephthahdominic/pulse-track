...
| `GET` | `/api/v1/analytics/live` | Currently active sessions (paginated `limit`/`offset`, true active count) |
| `GET` | `/api/v1/analytics/realtime` | Recent activity feed (paginated `limit`/`offset`) |
| `GET` | `/api/v1/analytics/realtime/clicks` | Live click rate (per-minute buckets over last `window` min) |
